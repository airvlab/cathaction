...
class CathDatasetAllFrames(Dataset):
    ...
    def _get_video_clips(self):
        ...
        for video_id in video_ids:
            ...
            while time_stop <= video_duration:
                clip = CathClip(video_id, time_start, time_stop - 1.0 / self.fps, fps=self.fps, 
                                video_duration=video_duration)
...
def get_datasets(args):
    ...
    if args.version == 'cathAction':
        dfs = {
            'train': get_cath_annotation(partition='train', **annotation_args),
            'validation': get_cath_annotation(partition='validation', **annotation_args),
            'test_s1': get_cath_annotation(partition='test_s1', **annotation_args),
            'test_s2': get_cath_annotation(partition='test_s2', **annotation_args),
        }
    else:
